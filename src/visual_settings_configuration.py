...
from matplotlib.colors import to_rgb, to_rgba, ListedColormap, Normalize
...
class VisualTabularConfiguration(VisualColorsConfiguration):

	def __init__(self):
		super().__init__()

	@staticmethod
	def get_swapped_row_and_column_parameters(number_rows, number_columns, row_labels, column_labels, cell_text, row_colors, column_colors, cell_colors):
		number_rows, number_columns = number_columns, number_rows
		row_labels, column_labels = column_labels, row_labels
		cell_text = list(
			zip(
				*cell_text))
		row_colors, column_colors = column_colors, row_colors
		cell_colors = list(
			zip(
				*cell_colors))
		parameters = (
	...
	@staticmethod
	def get_diagonal_table_colors(even_color, odd_color, number_rows, number_columns):
		cell_colors = list()
		for r in range(number_rows):
			...
			for c in range(number_columns):
				if r % 2 == 0:
					if c % 2 == 0:
						prime_color = even_color
					else:
						prime_color = odd_color
				else:
					if c % 2 == 0:
						prime_color = odd_color
					else:
			...
			cell_colors.append(
				row)
		# cell_colors = [
		# 	list(row_to_transpose)
		# 		for row_to_transpose in zip(
		# 			*cell_colors)]
		return cell_colors

	def autoformat_table(self, ax, table, size=None, dimensions=None):
		if size is None:
			size = float(
				self.cell_size)
		ax.axis(
		...
		if dimensions is not None:
			number_columns, number_rows = dimensions
			x_scale, y_scale = (
				number_columns,
				number_rows)
			table.scale(
...
class VisualLegendConfiguration(VisualTabularConfiguration):
